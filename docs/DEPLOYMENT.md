...
| Secret Manager (4 secrets) | Database URL, two API tokens, Gemini key. | ~$0.02/day |
| Gemini API | AI analysis. | per token; deterministic mode costs nothing |
...
gcloud sql instances create "$SQL_INSTANCE" \
  ...
  --no-assign-ip                       # private/socket access only
...
`--no-assign-ip` matters: without it the instance gets a public IP, and a
database holding failure evidence should not be reachable from the internet.
Cloud Run reaches it over the Cloud SQL Unix socket instead.
...
printf '%s' "$GEMINI_API_KEY"                | gcloud secrets create triagezero-gemini-key      --data-file=-
```
...
gcloud projects add-iam-policy-binding "$PROJECT_ID" \
  --member="serviceAccount:$API_SA" --role="roles/firebaseauth.viewer"

# read ONLY its own four secrets, not every secret in the project
for S in triagezero-database-url triagezero-ingestion-token \
         triagezero-dashboard-token triagezero-gemini-key; do
